...
>
> **注：文中图片已从原帖 CDN 下载保存至本地，版权归原作者所有。**
...
![小球地形模型动图](/images/ai-thinking-framework-terrain-ball.webp)
...
![语义漂移](/images/ai-thinking-framework-semantic-drift.png)

### 注意力稀释

地形塑造初期，明确的指令可以造成明显的地形变化，然而当空间的小球数量增加后，地形的张力会被逐渐拉平和稀释，原本的深沟坡度也会逐渐平缓；无意义的小球（修饰词，标点）也会参与平摊坡度。

![注意力稀释](/images/ai-thinking-framework-attention-dilution.png)

在长对话中模型会逐渐淡化提示词设定，因此需要反复强调核心指令，比如 `说中文`：

![反复强调核心指令](/images/ai-thinking-framework-repeat-instruction.png)

### 语义惯性

当对话持续是某一个风格或主题时，已有小球会在地表某个区域形成更深的峡谷，即更深的历史上下文状态，此时如果下一轮对话切换到另一个场景，小球极易受到上文的地形影响。

![语义惯性](/images/ai-thinking-framework-semantic-inertia.png)
...
![语义壁垒](/images/ai-thinking-framework-semantic-barrier.png)

> 思维链（Chain of Thought, COT）通过在陡峭的山坡上修筑起"阶梯平台"。后续小球每次只需滚向高一点的平台完成接力，并最终翻越山脊。

![COT 阶梯平台](/images/ai-thinking-framework-cot-steps.png)

### 相变

前面提到地形变化不是线性发生，可能在临界点引起整个地形的剧烈走向。比如一些看似毫无意义的废话最终在宏观表达上起到决定作用。

![相变](/images/ai-thinking-framework-phase-transition.png)
...
![特征纠缠](/images/ai-thinking-framework-feature-entanglement.png)

原本只想点亮逻辑性，但不可避免地激活了枯燥、学术废话等区域。

更明显的例子 三色韦恩图：

![三色韦恩图](/images/ai-thinking-framework-three-color-venn.png)
...
![显式配平](/images/ai-thinking-framework-explicit-trim.png)

显式表达很方便，不需要花很多精力管理上下文和注意力，但也存在很多弊端。

#### 知识冗余

![知识冗余](/images/ai-thinking-framework-knowledge-redundancy.png)
...
![注意力劫持](/images/ai-thinking-framework-attention-hijack.png)

#### 认知降维

运用显式的语义一方面会引导模型的注意力，另一方面也会将模型困于使用者的认知上限而失去了寻优能力。

![认知降维](/images/ai-thinking-framework-cognitive-dimension-reduction.png)
...
![隐式提纯](/images/ai-thinking-framework-implicit-purification.png)
...
![引导采样与回滚](/images/ai-thinking-framework-guided-sampling-rollback.png)
...
![逻辑符号掺杂剂](/images/ai-thinking-framework-dopant-logic.png)
...
![剧本标记掺杂剂](/images/ai-thinking-framework-dopant-script.png)

会计学追求实质重于形式，会讲求严格的资源权衡，我们可以预期会计学语义能强化对话中模型的资源分配逻辑。

![会计学掺杂剂](/images/ai-thinking-framework-dopant-accounting.jpeg)
...
![催化剂](/images/ai-thinking-framework-catalyst.png)

比如 `通俗易懂`，`奥卡姆剃刀`，`第一性原理`。

![催化剂示例](/images/ai-thinking-framework-catalyst-examples.png)
